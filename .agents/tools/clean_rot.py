...
Validates that the agent knowledge base (AGENTS.md + docs/) stays lean,
cross-linked, and free of drift. AGENTS.md is a map (~40 lines), not a manual.
Detailed docs live in docs/ and are only pulled in when needed.
...
def check_line_budget(agents_md, **_kw):
    ...
    if agents_md.exists():
        ...
        if count > MAX_AGENTS_LINES:
            warn(
                ...
                f"Move detail into docs/ and keep AGENTS.md as an index.",
            )
            warnings += 1
    else:
...
def check_broken_links(agents_md, docs_dir, repo_root, **_kw):
    """Verify all markdown links in AGENTS.md and docs/ point to files that exist."""
    warnings = 0
...
def check_orphan_docs(agents_md, docs_dir, repo_root, **_kw):
    """Every file in docs/ should be referenced from AGENTS.md."""
    warnings = 0
    ...
    for doc in sorted(docs_dir.glob("*.md")):
        ...
        if rel_path not in agents_text and f"./{rel_path}" not in agents_text:
            warn(
                ...
                f"add a link so agents can discover it.",
            )
            warnings += 1
...
def check_duplication(agents_md, docs_dir, repo_root, **_kw):
    """Flag near-duplicate content between AGENTS.md and docs/ files.
...
def check_stale_references(agents_md, docs_dir, repo_root, src_dir, **_kw):
    ...
    a slash for file paths, or start with 'world.' for dotted module paths). Bare
    ...
    for doc in all_docs:
        ...
        # e.g. `world/rpg/`, `world/parsing/tests/test_trie.py`
        # Skips bare filenames, tools, and evennia/ paths (those live in the venv).
        for match in re.finditer(
            r"`(world/[a-z_/]*(?:\.py)?/?)`", text
        ):
        ...
        # Check backtick-quoted Python dotted paths like `world.parsing.tests.test_trie`
        for match in re.finditer(r"`(world\.[a-z_.]+)`", text):
...
def check_index_density(agents_md, **_kw):
    ...
    if non_empty and prose / len(non_empty) > 0.50:
        warn(
            ...
            f"keep it as an index with pointers. Move prose to docs/.",
        )
        warnings += 1
